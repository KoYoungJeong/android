package com.tosslab.jandi.app.utils.analytics.sprinkler.model;

import com.tosslab.jandi.app.utils.analytics.sprinkler.PropertyKey;
import com.tosslab.jandi.app.utils.analytics.sprinkler.SprinklerEvents;
import com.tosslab.jandi.app.utils.analytics.sprinkler.model.Properties.ITopicId;

/**
 * Created by tee on 2016. 9. 9..
 */

public class SprinklrTopicLeave extends MainSprinklrModel
        implements ITopicId {

    private SprinklrTopicLeave() {
        super(SprinklerEvents.TopicLeave, true, true);
    }

    public static void sendFailLog(int errorCode) {
        new SprinklrTopicLeave().sendFail(errorCode);
    }

    public static void sendLog(long topicId) {
        new SprinklrTopicLeave()
                .setTopicId(topicId)
                .sendSuccess();
    }

    @Override
    public SprinklrTopicLeave setTopicId(long topicId) {
        setProperty(PropertyKey.TopicId, topicId);
        return this;
    }

}
