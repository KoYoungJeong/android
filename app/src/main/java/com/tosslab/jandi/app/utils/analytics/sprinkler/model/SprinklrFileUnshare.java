...
public class SprinklrFileUnshare extends MainSprinklrModel
        implements ITopicId, IFileId {

    private SprinklrFileUnshare() {
        super(SprinklerEvents.FileUnShare, true, true);
    }

    public static void sendFailLog(int errorCode) {
        new SprinklrFileUnshare().sendFail(errorCode);
    }

    public static void sendLog(long topicId, long fileId) {
        new SprinklrFileUnshare()
                .setTopicId(topicId)
                .setFileId(fileId)
                .sendSuccess();
    }
    ...
    @Override
    public SprinklrFileUnshare setFileId(long fileId) {
        setProperty(PropertyKey.FileId, fileId);
        return this;
    }

    @Override
    public SprinklrFileUnshare setTopicId(long topicId) {
