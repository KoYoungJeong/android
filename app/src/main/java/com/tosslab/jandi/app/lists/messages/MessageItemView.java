...
public class MessageItemView extends LinearLayout {
    ...
    @ViewById(R.id.iv_message_user_profile)
    ImageView mUserProfileImage;
    @ViewById(R.id.tv_message_user_name)
    TextView mUserName;
    @ViewById(R.id.tv_message_create_date)
    TextView mCreateTime;
    @ViewById(R.id.tv_message_content)
    TextView mMessageContent;

    // 댓글에 대한 표시일 경우
    @ViewById(R.id.vg_message_commented)
    RelativeLayout mLayoutMessageComment;
    @ViewById(R.id.tv_message_commented_owner)
    TextView mTextMessageCommentOwner;
    @ViewById(R.id.tv_message_commented_file_name)
    TextView mTextMessageCommentFileName;
    @ViewById(R.id.tv_message_commented_content)
    TextView mTextMessageCommentContent;

    // 내부 댓글일 경우
    @ViewById(R.id.vg_message_nested_comment)
    RelativeLayout mLayoutMessageNestedComment;
    @ViewById(R.id.tv_message_nested_comment_user_name)
    TextView mTextMessageNestedCommentUserName;
    @ViewById(R.id.tv_message_commented_create_date)
    TextView mTextMessageNestedCommentCreateDate;
    @ViewById(R.id.tv_message_nested_comment_content)
    TextView mTextMessageNestedCommentContent;


    // 이미지일 경우
    @ViewById(R.id.vg_message_image_file)
    RelativeLayout mLayoutMessageImageFile;
    @ViewById(R.id.tv_message_image_file_name)
    TextView mTextImageFileName;
    @ViewById(R.id.img_message_photo)
    ImageView mImagePhoto;
    @ViewById(R.id.tv_img_file_type)
    TextView mTextImageFileType;

    // 일반 파일
    @ViewById(R.id.vg_message_common_file)
    RelativeLayout mLayoutMessageCommonFile;
    @ViewById(R.id.tv_message_common_file_name)
    TextView mTextMessageCommonFileName;
    @ViewById(R.id.tv_common_file_type)
    ...
    private void showProfilePhoto(final MessageItem item) {
        ...
        mUserProfileImage.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                EventBus.getDefault().post(new ShowProfileEvent(item.getUserId()));
            }
        });
    }
