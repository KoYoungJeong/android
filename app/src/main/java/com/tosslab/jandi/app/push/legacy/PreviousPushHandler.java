...
public class PreviousPushHandler {
    private static PreviousPushHandler instance;
    private final PreviousJandiPushReceiverModel_ jandiPushReceiverModel;

    private PublishSubject<PushTO> pushSubject;


    public PreviousPushHandler() {
        this.pushSubject = PublishSubject.create();
        this.jandiPushReceiverModel = PreviousJandiPushReceiverModel_.getInstance_(JandiApplication.getContext());
        initPushQueue();
    }

    synchronized public static PreviousPushHandler getInstance() {
        if (instance == null) {
            instance = new PreviousPushHandler();
        }
        return instance;
    }
